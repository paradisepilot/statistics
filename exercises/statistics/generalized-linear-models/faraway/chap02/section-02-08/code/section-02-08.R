...
####################################################################################################
results.glm <- glm(formula = orientation ~ estrogen + androgen, data = hormone, family = binomial);
summary(results.glm);

####################################################################################################
temp.filename <- 'figure-02-04.png';
my.ggplot <- ggplot(data = NULL);

my.ggplot <- my.ggplot + geom_point(
	data    = hormone,
	mapping = aes(x = androgen, y = estrogen, colour = orientation)
	);

my.ggplot <- my.ggplot + geom_abline(
	intercept = - coef(results.glm)[['(Intercept)']] / coef(results.glm)[['estrogen']],
	slope     = - coef(results.glm)[['androgen']]    / coef(results.glm)[['estrogen']]
	);

my.ggplot <- my.ggplot + scale_x_continuous(limits = c(0,6), breaks = seq(0,6,1));
my.ggplot <- my.ggplot + scale_y_continuous(limits = c(0,6), breaks = seq(0,6,1));
...
ggsave(file = temp.filename, plot = my.ggplot, dpi = resolution, height = 6, width = 7.5, units = 'in');
