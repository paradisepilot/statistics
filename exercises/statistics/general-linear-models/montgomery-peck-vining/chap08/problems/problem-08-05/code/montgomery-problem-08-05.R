...
### GRAPHICS #######################################################################################
DF.temp <- DF.data[,c('mileage','weight','transmission.type')];
DF.temp[,'transmission.type'] <- as.factor(DF.temp[,'transmission.type']);

resolution <- 100;
graphics.format <- 'png';
my.filename <- paste('problem-08-05-exploratory',graphics.format,sep='.');
my.ggplot <- ggplot();
my.ggplot <- my.ggplot + xlim(-200,6200);
my.ggplot <- my.ggplot + ylim(0,40);
my.ggplot <- my.ggplot + geom_point(
        data     = DF.temp,
        mapping  = aes(x = weight, y = mileage, col = transmission.type)
        );
ggsave(file = my.filename, plot = my.ggplot, height = 0.5 * par("din")[1], dpi = resolution, units = 'in');

### (a) ############################################################################################
lm.full <- lm(
	formula = mileage ~ weight + transmission.type,
	data    = DF.data
	);

lm.reduced <- lm(
	formula = mileage ~ weight,
	data    = DF.data
	);

### manual verification
X <- as.matrix(cbind(
	intercept = rep(1,nrow(DF.data)),DF.data[,c('weight','transmission.type')]
	));
...
X.reduced <- as.matrix(cbind(
	intercept = rep(1,nrow(DF.data)),DF.data[,'weight']
	));
...
resolution <- 100;
graphics.format <- 'png';
my.filename <- paste('problem-08-05-a',graphics.format,sep='.');
my.ggplot <- ggplot();
my.ggplot <- my.ggplot + xlim(-200,6200);
my.ggplot <- my.ggplot + ylim(0,40);
my.ggplot <- my.ggplot + geom_point(
...
X <- as.matrix(cbind(
	X,displacement_transmission.type = X[,'weight'] * X[,'transmission.type']
	));
...
lm.full <- lm(
	formula = mileage ~ weight + transmission.type + weight*transmission.type,
	data    = DF.data
	);
...
resolution <- 100;
graphics.format <- 'png';
my.filename <- paste('problem-08-05-b',graphics.format,sep='.');
my.ggplot <- ggplot();
my.ggplot <- my.ggplot + xlim(-200,6200);
my.ggplot <- my.ggplot + ylim(0,40);
my.ggplot <- my.ggplot + geom_point(
