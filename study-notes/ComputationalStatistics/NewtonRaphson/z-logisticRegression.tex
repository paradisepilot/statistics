...
\item\label{logisticScoreEqn}
	...
	%\begin{equation*}
	%\overset{n}{\underset{i=1}{\sum}}\;\, x^{(i)}_{k} \cdot \left(\,\overset{{\color{white}+}}{y}_{i} - \pi_{i}(\beta)\right)
	%\;\; = \;\; 0\,,
	%\quad
	...
	%\end{equation*}
\item
	The derivative of $\pi : \Re^{p} \longrightarrow \Re^{n}$ with respect to $\beta \in \Re^{p}$
	is the map \,$\nabla\pi : \Re^{p} \longrightarrow \Re^{n \times p}$\, given by
	\begin{equation*}
	\left(\,\nabla\pi\,\right)(\beta)
	\;\; = \;\;
		D(\beta) \cdot X\,,
	...
	D(\beta) \; := \; \diag\!\left(\;
		...
		\Re^{n \times n}\,.
	\end{equation*}
	Consequently, the Hessian of the log-likelihood \,$\log L$\, is given by:
	\begin{equation*}
	\nabla_{\beta}^{2}\!\left(\,\overset{{\color{white}.}}{\log}\,L\,\right)
	\;\; = \;\;
		-\, X^{T} \cdot \left(\,\nabla\pi\,\right)(\beta)
	\;\; = \;\;
