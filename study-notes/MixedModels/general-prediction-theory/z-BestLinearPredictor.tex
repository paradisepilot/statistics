...
\begin{enumerate}
\item
	$\blp(\,Y\vert X)$ in fact does not depend on the particular choice of $\beta$.
\item
	$\blp(\,Y\vert X)$ is the ``best linear predictor of \,$Y$ in terms of $X$,'' in the following sense:
	\begin{equation*}
	E\!\left[\;\left(\,Y \overset{{\color{white}.}}{-} \blp(\,Y\vert X)\,\right)^{2}\;\right]
	\;\;\leq\;\;
	E\!\left[\;\left(\,Y \overset{{\color{white}.}}{-} f(X)\,\right)^{2}\;\right]\,,
	\quad
	\textnormal{for every affine function $f:\Re^{p}\longrightarrow\Re$}.
	\end{equation*}
\end{enumerate}
\end{theorem}
\proof
