...
\item
	$n, p \in \N$\, are natural numbers.
\item
	$Y^{(1)}, \ldots, Y^{(n)} : \Omega \longrightarrow \left\{0,1\right\}$\,
	are binary random variables, and
\item
	$X^{(1)}, \ldots, X^{(n)} : \Omega \longrightarrow \left\{0,1\right\}^{p} := \{0,1\} \times \{0,1\} \times \cdots \times \{0,1\}$\,
	are $\{0,1\}^{p}$-valued random variables.
\item
	$x^{(1)}, \ldots, x^{(n)} \in \left\{0,1\right\}^{p}$\,.
\end{itemize}
Suppose:
\begin{itemize}
\item
	The $\{0,1\}^{p+1}$-valued random variables
...
\item
	...
	\pi_{i}
	\;\; := \;\;
		P\!\left(\;Y^{(i)} = 1 \,\left\vert\; X^{(i)} = x^{(i)} \right.\right)
	\;\; = \;\;
		E\!\left[\; Y^{(i)}  = 1 \,\left\vert\; X^{(i)} = x^{(i)} \right.\right]
	\;\; = \;\;
		\dfrac{
...
\begin{enumerate}
\item
	The logarithm of the joint conditional probability distribution of
	\;$Y^{(i)}$,\,
	given \,$X^{(i)} = x^{(i)}$,\,
	...
	\log\,L
		&=&
		\log\,P\!\left(\;
			Y^{(1)}=y_{1},\ldots,Y^{(n)}=y_{n} \;\left\vert\; X^{(1)}=x^{(1)}, \ldots, X^{(n)}=x^{(n)} \right.
			\;\right)
	\\
	&=&
		{\color{white}+} \;\;
