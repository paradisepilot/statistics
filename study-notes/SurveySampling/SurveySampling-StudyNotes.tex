...
\section{Linear estimators for population totals}
\setcounter{theorem}{0}
...
We will use the common notation $y_{k}$ for $y(k)$.
We wish to estimate $T_{y} := \sum_{k \in U} y_{k}$ via survey sampling.
...
\vskip 0.1cm
\noindent
A random variable $\widehat{T}_{y} : \mathcal{S} \longrightarrow \Re$ is said to be
\underline{\emph{linear} in the population parameter $y : U \longrightarrow \Re$}
if it has the following form:
\begin{equation*}
\begin{array}{cccl}
\widehat{T}_{y} : & \mathcal{S} & \longrightarrow & \Re \\
     & s &\longmapsto & \underset{k\in s}{\sum}\,w_{k}(s)\,y_{k},
\end{array}
\end{equation*}
where, for each $k \in U$, $w_{k} : \mathcal{S} \longrightarrow \Re$ is itself
an $\Re$-valued random variable.
We call the $w_{k}$'s the \underline{\emph{weights}} of $\widehat{T}_{y}$.
\end{definition}
...
\vskip 0.2cm
\noindent
Let $\widehat{T}_{y;w} : \mathcal{S} \longrightarrow \Re$,
with $\widehat{T}_{y;w}(s) = \underset{k \in s}{\sum}\,w_{k}(s)\,y_{k}$,
be a random variable linear in the population parameter $y : U \longrightarrow \Re$.
Then,
\begin{equation*}
E\!\left[\;\widehat{T}_{y;w}\;\right] \; = \; T_{y},\;\textnormal{for arbitrary $y$}
\quad\Longleftrightarrow\quad
\sum_{s\ni k}\,p(s)\,w_{k}(s) \; = \; 1,\;\textnormal{for each $k \in U$}.
\end{equation*}
\end{proposition}
\proof
For each $k \in U$, define the indicator random variable
$I_{k} : \mathcal{S} \longrightarrow \{0,1\}$ by:
\begin{equation*}
I_{k}(s)
\;\;=\;\;
\left\{
...
Note that
\begin{equation*}
E\!\left[\;\widehat{T}_{y;w}\;\right]
\;=\; E\!\left[\,\underset{k \in s}{\sum}\,w_{k}\,y_{k}\,\right]
\;=\; E\!\left[\,\underset{k \in U}{\sum}\,I_{k}w_{k}\,y_{k}\,\right]
\;=\; \underset{k \in U}{\sum}\,E\!\left[\,I_{k}w_{k}\,\right]\,y_{k}
\end{equation*}
Hence, since $y : U \longrightarrow \Re$ is arbitrary,
\begin{equation*}
E\!\left[\;\widehat{T}_{y;w}\;\right] \;=\; T_{y} \; := \; \underset{k \in U}{\sum}\,y_{k}
\quad\Longleftrightarrow\quad
\sum_{k \in U}\left(E\!\left[\,I_{k}w_{k}\,\right]-1\right)\cdot y_{k} \; = \; 0
\quad\Longleftrightarrow\quad
E\!\left[\,I_{k}w_{k}\,\right] \; = \; 1,\;\textnormal{for each $k \in U$}
\end{equation*}
Lastly, note that
\begin{equation*}
E\!\left[\,I_{k}w_{k}\,\right]
\;\;=\;\; \sum_{s \in \mathcal{S}}\,p(s)\,I_{k}(s)\,w_{k}(s)
\;\;=\;\; \sum_{k \ni s}\,p(s)\,w_{k}(s),
\end{equation*}
...
\vskip 0.2cm
\noindent
For a sampling design each of whose inclusion probabilities is strictly positive,
the Horvitz-Thompson estimator $\widehat{T}^{\textnormal{HT}}_{y}$ is well-defined
and it is the unique unbiased estimator for $T_{y}$, for arbitrary $y$, which is linear in $y$ and
whose weights are constants in $s$.
\end{corollary}
...
\end{equation*}
where $\pi_{k} := \underset{s \ni k}{\sum}\,p(s)$ is the inclusion probability of $k \in U$
under the sampling design $p : \mathcal{S} \longrightarrow (0,1]$.
Clearly, $\widehat{T}^{\textnormal{HT}}_{y}$ is linear in $y$ with weights constant in $s$.
Next, note that:
\begin{eqnarray*}
E\!\left[\;\widehat{T}^{\textnormal{HT}}_{y}\;\right]
&=& E\!\left[\;\sum_{k \in s}\,\dfrac{y_{k}}{\pi_{k}}\;\right]
\;\;=\;\; E\!\left[\;\sum_{k \in U}\,I_{k}\,\dfrac{y_{k}}{\pi_{k}}\;\right]
\;\;=\;\; \sum_{k \in U}\,E\!\left[\;I_{k}\;\right]\,\dfrac{y_{k}}{\pi_{k}}
\;\;=\;\; \sum_{k \in U}\,\left[\,\sum_{s\in\mathcal{S}}\,p(s)I_{k}(s)\,\right]\,\dfrac{y_{k}}{\pi_{k}} \\
&=& \sum_{k \in U}\,\left[\,\sum_{s \ni k}\,p(s)\,\right]\,\dfrac{y_{k}}{\pi_{k}}
\;\;=\;\; \sum_{k \in U}\,\pi_{k}\,\dfrac{y_{k}}{\pi_{k}}
\;\;=\;\; \sum_{k \in U}\,y_{k} \;\; = \;\; T_{y}
\end{eqnarray*}
Hence, $\widehat{T}^{\textnormal{HT}}_{y}$ is an unbiased estimator for $T_{y}$.
Conversely, let
\begin{equation*}
\widehat{T}_{y;w}(s) \; = \; \sum_{k \in s}\,w_{k}\,y_{k}
\end{equation*}
be any unbiased estimator for $T_{y}$ which linear in $y$ with weights constant in $s$.
Thus,
\begin{equation*}
\sum_{k \in U}\,y_{k}
\;=\; T_{y}
\;=\; E\!\left[\;\widehat{T}_{y;w}\;\right]
\;=\; E\!\left[\;\sum_{k \in s}\,w_{k}\,y_{k}\;\right]
\;=\; E\!\left[\;\sum_{k \in U}\,I_{k}\,w_{k}\,y_{k}\;\right]
\;=\; \sum_{k \in U}\,E\!\left[\;I_{k}\;\right]\,w_{k}\,y_{k}
\;=\; \sum_{k \in U}\,\pi_{k}\,w_{k}\,y_{k}.
\end{equation*}
Since $y$ is arbitrary, the above equation immediately implies that
\begin{equation*}
\pi_{k}w_{k} - 1 = 0,
\end{equation*}
or equivalently, $w_{k} = \dfrac{1}{\pi_{k}}$; in other words,
$\widehat{T}_{y;w}$ is in fact equal to the Horvitz-Thompson estimator.
...
with $\widehat{T}_{y;w}(s) = \underset{k \in s}{\sum}\,w_{k}(s)\,y_{k}$,
be a random variable linear in the population parameter $y : U \longrightarrow \Re$.
Then, the variance of $\widehat{T}_{y;w}$ is given by:
\begin{equation*}
\Var\!\left[\;\widehat{T}_{y;w}\;\right]
\;\;=\;\;
\sum_{i \in U}a_{i}\,y_{i}^{2} + \underset{i \neq j}{\sum_{i,j \in U}}\,a_{ij}\,y_{i}\,y_{j},
\end{equation*}
where
\begin{equation*}
...
\begin{eqnarray*}
\Var\!\left[\;\widehat{T}_{y;w}\;\right]
&=& \Cov\!\left[\;\sum_{i\in U}\,I_{i}\,w_{i}\,y_{i}\;,\;\sum_{k\in U}\,I_{k}\,w_{k}\,y_{k}\;\right]
\;\;=\;\; \sum_{i\in U}\,\sum_{k\in U}\,\Cov\!\left[\;I_{i}\,w_{i}\,,\,I_{k}\,w_{k}\;\right]\,y_{i}\,y_{k} \\
&=&    \sum_{k\in U}\Var\!\left[\;I_{k}\,w_{k}\;\right] y_{k}^{2}
         \;\;+\;\; \underset{i \neq k}{\sum_{i,k\in U}}\Cov\!\left[\;I_{i}\,w_{i}\,,\,I_{k}\,w_{k}\;\right]\,y_{i}\,y_{k} \\
\end{eqnarray*}
Furthermore, if $\widehat{T}_{y;w}$ is an unbiased estimator for $T_{y} := \underset{k\in U}{\sum}\,y_{k}$
for arbitrary $y$, then $\underset{s \ni k}{\sum}\,p(s)\,w_{k}(s) = 1$, for each $k \in U$, by the preceding
Proposition. Hence, for each $k \in U$,
\begin{eqnarray*}
a_{k} &:=& \Var\!\left[\;I_{k}\,w_{k}\;\right]
\;\;=\;\; E\!\left[\;I_{k}^{2}\,w_{k}^{2}\;\right] - E\!\left[\;I_{k}\,w_{k}\;\right] \\
&=&\left(\sum_{s\in\mathcal{S}}\,p(s)\,I^{2}_{i}(s)\,w_{i}^{2}(s)\right) - \left(\sum_{s\in\mathcal{S}}\,p(s)\,I_{i}(s)\,w_{i}(s)\right)
\;\;=\;\;\left(\sum_{s\in\mathcal{S}}\,p(s)\,I^{2}_{i}(s)\,w_{i}^{2}(s)\right) - \left(\sum_{s \ni k}\,p(s)\,w_{i}(s)\right) \\
&=&\left(\sum_{s\in\mathcal{S}}\,p(s)\,I_{i}(s)\,w_{i}^{2}(s)\right) - 1,
\end{eqnarray*}
and the proof of the Proposition is complete. \qed
...
\vskip 0.1cm
\noindent
Let $\widehat{T}_{y;w} : \mathcal{S} \longrightarrow \Re$ be a random variable 
which is linear in the population parameter $y : U \longrightarrow \Re$, i.e.
\begin{equation*}
\begin{array}{cccl}
\widehat{T}_{y;w} : & \mathcal{S} & \longrightarrow & \Re \\
     & s &\longmapsto & \underset{k\in s}{\sum}\,w_{k}(s)\,y_{k},
\end{array}
\end{equation*}
where, for each $k \in U$, $w_{k} : \mathcal{S} \longrightarrow \Re$ is itself
an $\Re$-valued random variable.
Let $x : U \longrightarrow \Re$ be another population parameter and $T_{x} := \underset{k\in U}{\sum}\,x_{k}$.
Then, $\widehat{T}_{y;w}$ is said to be
...
\begin{proposition}
\label{proposition:MSE}
\mbox{}
\vskip 0.1cm
\noindent
Let $\widehat{T}_{y;w,x} : \mathcal{S} \longrightarrow \Re$ be a random variable 
which is linear in the population parameter $y : U \longrightarrow \Re$ and
calibrated with respect to the population parameter $x : U \longrightarrow \Re$,
with $x_{k} > 0$ for each $k \in U$.
Then, the mean squared error of $\widehat{T}_{y;w,x}$ as an estimator of $T_{y}$ is given by:
\begin{equation*}
\MSE\!\left[\;\widehat{T}_{y;w,x}\;\right]
\;=\;
- \dfrac{1}{2}\,\underset{i \neq k}{\sum_{i,k\in U}}\,a_{ik}\left(\dfrac{y_{i}}{x_{i}} - \dfrac{y_{k}}{x_{k}}\right)^{2}\,x_{i}\,x_{k},
...
\begin{eqnarray*}
\MSE\!\left[\;\widehat{T}_{y;w,x}\;\right]
&=& E\!\left[\;\left(\widehat{T}_{y;w,x} - T_{y}\right)^{2}\;\right]
\;\;=\;\; E\!\left[\;\left(\sum_{k\in U}y_{k}(I_{k}w_{k}-1)\right)\left(\sum_{i\in U}y_{i}(I_{i}w_{i}-1)\right)\;\right] \\
&=& \sum_{k\in U}\,\sum_{i\in U}\,E\!\left[\;\left(I_{k}w_{k}-1\right)\left(I_{i}w_{i}-1\right)\;\right]\,y_{k}\,y_{i}
\;\;=\;\; \sum_{k \in U}\,a_{kk}\,y_{k}^{2} + \underset{i\neq k}{\sum_{i,k\in U}}\,a_{ik}\,y_{i}\,y_{k} \\
&=& \sum_{k \in U}\,a_{kk}\left(\dfrac{y_{k}}{x_{k}}\right)^{2}x_{k}^{2}
+ \underset{i\neq k}{\sum_{i,k\in U}}\,a_{ik}\left(\dfrac{y_{i}}{x_{i}}\right)\left(\dfrac{y_{k}}{x_{k}}\right)x_{i}\,x_{k}
\end{eqnarray*}
On the other hand,
\begin{eqnarray*}
&&   - \dfrac{1}{2}\,\underset{i \neq k}{\sum_{i,k\in U}}\,a_{ik}\left(\dfrac{y_{i}}{x_{i}} - \dfrac{y_{k}}{x_{k}}\right)^{2}\,x_{i}\,x_{k} \\
&=& - \dfrac{1}{2}\,\underset{i \neq k}{\sum_{i,k\in U}}\,a_{ik}
\left[\left(\dfrac{y_{i}}{x_{i}}\right)^{2} - 2\left(\dfrac{y_{i}}{x_{i}}\right)\left(\dfrac{y_{k}}{x_{k}}\right) + \left(\dfrac{y_{k}}{x_{k}}\right)^{2}\right]\,x_{i}\,x_{k} \\
&=& - \dfrac{1}{2}\,\underset{i \neq k}{\sum_{i,k\in U}}\,a_{ik}
\left[\left(\dfrac{y_{i}}{x_{i}}\right)^{2} + \left(\dfrac{y_{k}}{x_{k}}\right)^{2}\right]\,x_{i}\,x_{k}
+ \underset{i \neq k}{\sum_{i,k\in U}}\,a_{ik}\left(\dfrac{y_{i}}{x_{i}}\right)\left(\dfrac{y_{k}}{x_{k}}\right)x_{i}\,x_{k}
\end{eqnarray*}
...
\sum_{k \in U}\,a_{kk}\left(\dfrac{y_{k}}{x_{k}}\right)^{2}x_{k}^{2}
\;\;=\;\;
- \dfrac{1}{2}\,\underset{i \neq k}{\sum_{i,k\in U}}\,a_{ik}\left[\left(\dfrac{y_{i}}{x_{i}}\right)^{2} + \left(\dfrac{y_{k}}{x_{k}}\right)^{2}\right]\,x_{i}\,x_{k},
\end{equation*}
which is equivalent to:
\begin{equation}\label{MSE}
\sum_{i\in U}\,\sum_{k\in U}\,a_{ik}\left[\left(\dfrac{y_{i}}{x_{i}}\right)^{2} + \left(\dfrac{y_{k}}{x_{k}}\right)^{2}\right]\,x_{i}\,x_{k}
\;\;=\;\; 0.
\end{equation}
Observe that
\begin{eqnarray*}
...
&=&
\sum_{i\in U}\,\sum_{k\in U}\,a_{ik}\left(\dfrac{y_{i}}{x_{i}}\right)^{2}x_{i}\,x_{k}
+
\sum_{i\in U}\,\sum_{k\in U}\,a_{ik}\left(\dfrac{y_{k}}{x_{k}}\right)^{2}x_{i}\,x_{k}
\;\;=\;\;
2\,\sum_{i\in U}\,\sum_{k\in U}\,a_{ik}\left(\dfrac{y_{i}}{x_{i}}\right)^{2}x_{i}\,x_{k} \\
&=& 2\,\sum_{i\in U}\,x_{i}\left(\dfrac{y_{i}}{x_{i}}\right)^{2}\left(\sum_{k\in U}\,a_{ik}x_{k}\right).
\end{eqnarray*}
...
\begin{proposition}[The Yates-Grundy-Sen Variance Estimator]
\mbox{}
\vskip 0.1cm
\noindent
...
calibrated with respect to the population parameter $x : U \longrightarrow \Re$,
with $x_{k} > 0$ for each $k \in U$.
Suppose that $\widehat{T}_{y;w,x}$ is an unbiased estimator for
$T_{y} := \underset{k \in U}{\sum}y_{k}$, for arbitrary $y$.
Then, the following is an unbiased estimator of the variance
$\Var\!\left[\;\widehat{T}_{y;w,x} \;\right]$ of $\widehat{T}_{y;w,x} $:
For each $s \in \mathcal{S}$ admissible in the sampling design $p : \mathcal{S} \longrightarrow (0,1]$,
\begin{equation*}
\widehat{\Var}\!\left[\;\widehat{T}_{y;w,x} \;\right]\!(s)
\;\;:=\;\;
-\dfrac{1}{2}\underset{i\neq k}{\sum_{i,k\in s}}\left(w_{i}(s)w_{k}(s) - \dfrac{1}{\pi_{ik}}\right)\left(\dfrac{y_{i}}{x_{i}} - \dfrac{y_{k}}{x_{k}}\right)^{2}x_{i}\,x_{k}
\end{equation*}
\end{proposition}

\noindent
\textbf{Terminology:}\quad
$\widehat{\Var}\!\left[\;\widehat{T}_{y;w,x} \;\right]$ is called the Yates-Grundy-Sen Variance Estimator.

\vskip 0.5cm
\proof
Since $\widehat{T}_{y;w,x}$ is an unbiased estimator for $T_{y}$ by hypothesis, we have
$\Var\!\left[\;\widehat{T}_{y;w,x}\;\right] = \MSE\!\left[\;\widehat{T}_{y;w,x}\;\right]$.
By Proposition \ref{proposition:MSE}, we thus have:
\begin{equation*}
\Var\!\left[\;\widehat{T}_{y;w,x}\;\right]
\;\;=\;\;
- \dfrac{1}{2}\,\underset{i \neq k}{\sum_{i,k\in U}}\,a_{ik}\left(\dfrac{y_{i}}{x_{i}} - \dfrac{y_{k}}{x_{k}}\right)^{2}\,x_{i}\,x_{k},
\quad\textnormal{where}\;\;
a_{ik} \;:=\; E\!\left[\;\left(I_{i}\,w_{i}-1\right)\left(I_{k}\,w_{k}-1\right)\;\right].
...
On the other hand,
\begin{eqnarray*}
E\!\left(\;\widehat{\Var}\!\left[\;\widehat{T}_{y;w,x} \;\right]\;\right)
&=& 
-\dfrac{1}{2}\underset{i \neq k}{\sum_{i,k\in U}}E\!\left[\;I_{i}I_{k}\left(w_{i}w_{k} - \dfrac{1}{\pi_{ik}}\right)\;\right]\left(\dfrac{y_{i}}{x_{i}} - \dfrac{y_{k}}{x_{k}}\right)^{2}x_{i}\,x_{k}
\end{eqnarray*}
...
&=& E\!\left[\;I_{i}\,I_{k}\,w_{i}\,w_{k}\;\right] - \sum_{s \ni i}\,p(s)\,w_{i} - \sum_{s \ni k}\,p(s)\,w_{k} + 1
...
the unbiasedness hypothesis on $\widehat{T}_{y;w,x}$ as an estimator for $T_{y}$.
The proof of the present Proposition is now complete.  \qed
