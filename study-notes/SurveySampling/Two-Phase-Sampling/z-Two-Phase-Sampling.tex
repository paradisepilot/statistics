...
\item
	...
	& = &
		...
		\; + \;
		E_{\,p^{(1)}}\!\!\left[\;
			\Var\!\left(\, \left.\widehat{T}^{\TvertO}_{y} \;\right\vert\, S_{1} \,\right)
	...
	\end{eqnarray*}
	where
	\,$\Var\!\left[\, \left.\widehat{T}^{\TvertO}_{y} \;\right\vert\, S_{1} \,\right] : \mathcal{S}^{(1)} \longrightarrow \Re$\,
	is given by
	\begin{equation*}
	\Var\!\left[\, \left.\widehat{T}^{\TvertO}_{y} \;\right\vert\, S_{1} = {\color{red}s_{1}} \,\right]
	\;\; = \;\;
		\underset{i \in {\color{red}s_{1}} }{\sum}\;\;
		\underset{j \in {\color{red}s_{1}} }{\sum}\;\,
			\Delta^{(2)}_{ij \vert {\color{red}s_{1}}}
			\cdot
	...
	&{\color{white}\overset{\textnormal{\Large$1$}}{111}}&
	\pi^{(2)}_{ij \vert s_{1}} & := & \underset{i,j \in s_{2}}{\sum}\; p^{(2)}_{s_{1}}(s_{2})\,.
	\end{array}
	\end{equation*}
\item
	If \,$\pi^{(1)}_{ij} > 0$\, for each \,$i,j \in U$\, and
	\,$\pi^{(2)}_{ij \vert s_{1}} > 0$\, for each \,$s_{1} \in \mathcal{S}^{(1)}$\, and each \,$i,j \in s_{1}$\,,
	then the function
