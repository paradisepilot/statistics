...
\begin{theorem}\label{theorem:rStage}
\mbox{}\vskip 0.1cm
\noindent
...
\begin{corollary}\label{corollary:threeStage}\mbox{}\vskip 0.1cm
\noindent
For a three-stage sampling design with invariant and independent subsampling,
...
First, recall that for a simple random sampling without replacement (SRSWOR),
...
\end{eqnarray}
With the above observations, applying Theorem \ref{theorem:rStage} recursively to
...
&+& w^{(3)}_{ia}\underset{c \in s^{(3)}_{ia}}{\textnormal{\Large$\sum$}}\widehat{V}^{(4)}_{iac}
\\ \\
\widehat{V}^{(4)}_{iac}
