...
\item
	...
	\begin{equation*}
	1 - \varepsilon
	\;\; \leq \;\; \underset{n\rightarrow\infty}{\liminf}\; P_{*}(\,X_{n} \in K^{\delta(m)}\,)
	\;\; \leq \;\; 1\,,
	\;\;
	\textnormal{for each \,$\delta(m) > 0$},
	\end{equation*}
	where the $\delta(m)$-enlargement $K^{\delta(m)}$ of $K$ is defined to be:
	\begin{equation*}
	K^{\delta(m)}
		\;\; := \;\;
		\left\{\;
...
\item
	...
	\end{equation*}
	By Lemma \ref{LemmaAEpsilon}, the set $K^{\delta(m)} \subset \D$
	is an open subset of $(\D,d)$, for each $\delta(m) > 0$.
	Hence, by the Portmanteau Theorem (Theorem \ref{TheoremPortmanteau}),
	we have
	\begin{equation*}
	P\!\left(\,X \in K^{\delta(m)}\,\right)
	\;\; \leq \;\;
		\underset{n\rightarrow\infty}{\liminf}\;
		P_{*}\!\left(\,X_{n} \in K^{\delta(m)}\,\right),
	\quad
	\textnormal{for each \,$\delta(m) > 0$}\,.
	\end{equation*}
	Thus, since $K \subset K^{\delta(m)}$, we see that
	\begin{equation*}
	1 - \varepsilon
	\;\; \leq \;\;
		P\!\left(\,X \in K\,\right)
	...
	1 - \varepsilon
	\;\;\leq\;\;
		\underset{n\rightarrow\infty}{\liminf}\;P_{*}\!\left(\,X_{n} \in K^{\delta(m)}\,\right)
	\;\;\leq\;\; 1\,,
	\quad
	\textnormal{for each \,$\delta(m) > 0$}\,.
	\end{equation*}
	...
	1 - \varepsilon
	\;\;\leq\;\;
		\underset{n\rightarrow\infty}{\liminf}\; P_{*}(X_{n} \in K^{\delta(m)})
	\;\;\leq\;\;
		\underset{n\rightarrow\infty}{\limsup}\; P_{*}(X_{n} \in K^{\delta(m)})
	\;\;\leq\;\;
		P\!\left(\,X \in \overline{K^{\delta(m)}}\;\right),
	\quad
	\textnormal{for each \,$\delta(m) > 0$}\,.
	\end{equation*}
	Letting \,$\delta(m) \downarrow 0$,\, we may conclude that
	\begin{equation*}
...
\vskip 0.3cm
\noindent
By the asymptotic tightness hypothesis, there exists a compact $K \subset \D$ such that
\begin{equation*}
1 - \varepsilon
\;\; \leq \;\;
...
Thus, there exists $m_{0} \in \N$ such that
\begin{equation*}
\vert\,f_{m}(x)\,\vert \leq \varepsilon,\,
\quad\textnormal{for each \,$m \geq m_{0}$\, and \,$x \in K$}.
\end{equation*}
Now, for each fixed $m \geq m_{0}$, the compactness of $K$ implies, by an easy argument,
that there exists $\delta(m) > 0$ such that
\,$\vert\,f_{m}(x)\,\vert \leq  2\,\varepsilon$,\, for each \,$x \in K^{\delta(m)}$.
%Next, we claim that, for each $m \geq m_{0}$, we have
%\begin{equation*}
%L(\,f_{m}\,)
%\;\; \leq \;\;
%	\left(\,2 \, \overset{{\color{white}.}}{+} \, \Vert\,f_{1}\,\Vert_{\infty} \,\right) \cdot \varepsilon
%\end{equation*}
Next, we argue that, for each $m \geq m_{0}$,
\begin{equation*}
E\!\left[\;
...
%1 - \varepsilon
%\;\; \leq \;\;
%	\underset{i\rightarrow\infty}{\lim}\; P_{*}\!\left(\,X_{n(i)} \in K^{\delta(m)}\,\right)
%\;\; \leq \;\;
%	1\,.
%\end{equation*}
Note that
\begin{eqnarray*}
1 - \varepsilon \; \leq \; \underset{i\rightarrow\infty}{\liminf}\; P_{*}\!\left(\,X_{n(i)} \in K^{\delta(m)}\,\right)
&\Longrightarrow&
	1 \,-\, \underset{i\rightarrow\infty}{\liminf}\; P_{*}\!\left(\,X_{n(i)} \in K^{\delta(m)}\,\right) \;\leq\; \varepsilon
\\
&\Longrightarrow&
	1 \,+\, \underset{i\rightarrow\infty}{\limsup}\left(\,
		\overset{{\color{white}1}}{-}\,P_{*}\!\left(\,X_{n(i)} \in K^{\delta(m)}\,\right)
	\,\right)
	\;\leq\; \varepsilon
\\
&\Longrightarrow&
	\underset{i\rightarrow\infty}{\limsup}\left(\;
		1 \overset{{\color{white}.}}{-} P_{*}\!\left(\,X_{n(i)} \in K^{\delta(m)}\,\right)
	\,\right)
	\;\leq\; \varepsilon
\\
&\Longrightarrow&
...
\end{eqnarray*}
Choose any $A_{n(i)} \in \mathcal{A}_{n(i)}$ such that
\begin{equation*}
...
\;\; \leq \;\;
	2\,\varepsilon\,,
\quad
\textnormal{on \,$A_{n(i)} \,\cap\, B_{m,n(i)}$}
\end{equation*}
Thus, 
\begin{eqnarray*}
E\!\left[\;
...
\\
&=&
	\int_{A_{n(i)} \,\cap\, B_{m,n(i)}}
		\left(\;f_{m} \circ X_{n(i)}\,\right)_{*}
	\;\d\mu_{n(i)}
\\
&=&
	\int_{A_{n(i)} \,\cap\, B_{m,n(i)}} 2\,\varepsilon \;\d\mu_{n(i)}
\;\; \leq \;\;
	2\,\varepsilon
\end{eqnarray*}
