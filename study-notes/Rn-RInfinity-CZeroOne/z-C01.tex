...
\item
	We prove (iii) by exhibiting $P_{0}, P_{1}, P_{2},\,\ldots \, \in \mathcal{M}_{1}\!\left(\,\Czo\,\right)$
	such that $P_{n}$ does NOT converge weakly to $P_{0}$ as $n \longrightarrow \infty$, but
	\begin{equation*}
	\lim_{n\rightarrow\infty} P_{n}\!\left(A\right) \,=\, P_{0}(A),
	\quad\textnormal{for each $A \in \mathcal{B}_{f}(\Czo)$},
	\end{equation*}
	in particular, for each $P_{0}$-continuity set in $\mathcal{B}_{f}(\Czo)$.
