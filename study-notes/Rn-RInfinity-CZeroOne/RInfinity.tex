...
\item
	...
	\vskip 0.1cm
	\noindent
	\underline{$\mathcal{B}_{f}\!\left(\Re^{\infty}\right)$ is closed under finite intersections}\quad
	...
	We need to establish the reverse inclusion, which will immediately follows from:
	\begin{center}
	...
	%By Theorem \ref{MetricSpacePropertiesRInfinity}\eqref{FiniteDimensionalSetsFormBasisInRInfinity},
	%$\mathcal{B}_{f}\!\left(\Re^{\infty}\right)$ contains a basis for the topology of $\Re^{\infty}$.
	By Theorem \ref{MetricSpacePropertiesRInfinity}\eqref{RInfinitySeparable},
	$\Re^{\infty}$ is separable.
