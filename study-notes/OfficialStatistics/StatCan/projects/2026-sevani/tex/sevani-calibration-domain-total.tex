...
\section{Variance estimation methodology for {\color{red}calibration} estimators with composite linear imputation for {\color{red}domain totals}}
...
\item
    ...
    is the target $\Re$-valued population characteristic.
    \vskip 0.1cm
    We remind ourselves here that, if there are no missing data for the units in the selected sample,
    then the \textit{Horvitz-Thompson population total estimator}
    \begin{eqnarray*}
    \widehat{T}_{d,Y}
    & = &
...
\vskip 1.0cm
\subsection{Probabilistic framework of SEVANI \cite{Beaumont2011}}\label{ProbabilisticFramework}
\setcounter{theorem}{0}
...
\item
    $\Cdotdot\,=\,\left(\,\overset{{\color{white}.}}{C_{ij}}\,\right) : \Omega \longrightarrow \Re^{N \times q}$\,
...
\item
    $\Rdot \,=\, (R_{1},R_{2},\ldots,R_{N})^{T} : {\color{black}\Omega \otimes \mathcal{P}(U)} \longrightarrow \{0,1\}^{N \times 1}$\,
...
\item
    $\left(\,\Ydot\,,\Cdotdot\,,\Xdotdot\,\right)$\, and \,$\Sdot$\, are (unconditionally) independent.
\item
    $\Ydot$\, and \,$(\,\Sdot\,,\Rdot\,)$\,
    are {\color{red}conditionally independent} given
    \,$(\,\Cdotdot\,,\Xdotdot\,)$.
\item
    $\Cov\!\left(\;Y_{k}\,,\,Y_{l}\;\left\vert\;\Cdotdot\,,\Xdotdot\right.\right) \,=\, 0$,\,
