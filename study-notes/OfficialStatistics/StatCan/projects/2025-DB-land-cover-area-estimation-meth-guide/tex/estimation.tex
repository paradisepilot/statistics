...
\subsection{Area estimates for reference land cover types by ecoprovince}
...
\,$T_{p}^{(r)}$'s\,
by {\color{black}calibrating} the
\,$T_{p}^{(r)}$’s\,
...
\columnbreak
    ...
    \;\, = \;\,
        ...
        \textnormal{\small number of all grid cells}
        \\
        \textnormal{\small in sample \,$\mathcal{S}_{p,k}$}
        \end{array}\!\!\right\}
    \\
    y^{(r)}_{i}
    & := &
...
\subsection{Area estimates for reference land cover types by ecozones}
...
\subsection{Area estimates for reference land cover types over inland Canada}
These were generated analogously as their ecoprovince and ecozone counterparts.
Since inland Canada is partitioned into ecozones,
the area estimate for reference land cover type \,$r$\, over inland Canada
...
reference land cover type \,$r$\, over inland Canada
is the sum of the variance estimates of its summands (summing over all ecozones). 

          %%%%% ~~~~~~~~~~~~~~~~~~~~ %%%%%

\subsection{Area estimates for reference land cover types by drainage regions}
...
regions.
The preceding approach of generating ecozone-level estimates from ecoprovince-level ones
(straightforward addition of point and variance estimates of constituent ecoprovinces)
is not applicable.
...
\vskip -0.3cm
\input{tex/StrRS-HT-drainage-regions}

          %%%%% ~~~~~~~~~~~~~~~~~~~~ %%%%%

\subsection{Area estimates for reference land cover types by sub-drainage areas or by major 
drainage areas}
The aforementioned area estimation approach for drainage regions $\times$
...
There are two data tables in the release \cite{StatCanCoELandCoverArea2025}:
\renewcommand{\theenumi}{(\roman{enumi})}
\renewcommand{\labelenumi}{\mbox{}\;\theenumi$\;$}
\begin{enumerate}
\item
    area estimates by reference land cover type and
    ecoprovince/ecozone \cite{StatCanCoELandCoverAreaEcoprovinces}, and
\item
    area estimates by reference land cover type and
    drainage region/sub-drainage area/major drainage area \cite{StatCanCoELandCoverAreaDrainageRegions}.
\end{enumerate}
Note that:
\begin{itemize}
\item
    both tables contain land-cover-specific area estimates for inland Canada, and
\item
...
to ensure that, for each given ecoprovince,
the area estimates of different reference land cover types
within that ecoprovince
