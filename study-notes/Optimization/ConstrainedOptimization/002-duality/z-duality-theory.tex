...
\noindent
\begin{definition}[The Primal Problem]
\mbox{}
\vskip 0.1cm
\noindent
Suppose:
...
\noindent
\begin{definition}[The Lagrangian function and the Dual Problem]
\mbox{}
\vskip 0.1cm
\noindent
Suppose:
...
\vskip 1.0cm
\begin{theorem}[Weak Duality/Duality Gap]\label{WeakDuality}
\mbox{}
\vskip 0.1cm
\noindent
...
\vskip 1.0cm
\begin{proposition}[Existence of geometric multipliers gives characterization of Primal Problem solutions]
...
%\vskip 1.0cm
\clearpage
\begin{theorem}[(Primal Problem Solution, Geometric multiplier)-pairs]
\label{PrimalSolutionGeometricMultiplierPairs}
\mbox{}
\vskip 0.1cm
\noindent
...
\quad\Longleftrightarrow\quad\;
	\underset{\mu\,\in\Re_{\geq 0}^{r}}{\sup}\left\{\,\overset{{\color{white}.}}{L(x^{*}\!,\mu)}\,\right\}
	\;\; = \;\;
	L(x^{*}\!,\mu^{*})
	\;\; = \;\;
	\underset{x\,\in\,X}{\inf}\left\{\,\overset{{\color{white}.}}{L(x,\mu^{*})}\,\right\}
...
\vskip 0.2cm
\noindent
	The Saddle Point Theorem (Theorem \ref{TheoremSaddlePoint}) illustrates that,
	for each (Primal Solution, Geometric Multiplier) pair \,$(x^{*},\mu^{*})$,\,
	the Primal Solution \;$x^{*}$ in fact also solves
	\begin{center}
	the inner unconstrained minimization problem of the Dual Problem corresponding to \,$\mu^{*}$.
	\end{center}
	We explain in more details: Note that Theorem \ref{TheoremSaddlePoint} states that
	\,$(x^{*},\mu^{*}) \,\in\, X \times \Re_{\geq 0}^{r}$\,
	is a (Primal Solution, Geometric Multiplier) pair if and only if
	equality in fact holds throughout the following series of inequalities:
	\begin{eqnarray*}
		{\color{red}\underset{x\,\in\,\Omega(X,g)}{\inf}\left\{\;
		\overset{{\color{white}.}}{f(x)}
		\;\right\}}
	& = &
		\underset{x\,\in X}{\inf}\left\{\,\underset{\mu\,\in\Re_{\geq 0}^{r}}{\sup}\,\overset{{\color{white}.}}{L(x,\mu)}\,\right\}
	\\
	& \leq &
		\underset{\mu\,\in\Re_{\geq 0}^{r}}{\sup}\left\{\,\overset{{\color{white}.}}{L(x^{*}\!,\mu)}\,\right\}
		\;\; = \;\;
		L(x^{*}\!,\mu^{*})
		\;\; = \;\;
		{\color{red}\underset{x\,\in\,X}{\inf}\left\{\,\overset{{\color{white}.}}{L(x,\mu^{*})}\,\right\}}
	\\
	& \leq &
		\underset{\mu\,\in\Re_{\geq 0}^{r}}{\sup}\left\{\,\underset{x\,\in\,X}{\inf}\;\overset{{\color{white}.}}{L(x,\mu)}\,\right\}
	\;\; \leq \;\;
		\underset{x\,\in X}{\inf}\left\{\,\underset{\mu\,\in\Re_{\geq 0}^{r}}{\sup}\,\overset{{\color{white}.}}{L(x,\mu)}\,\right\},
	\end{eqnarray*}
	where the last equality is Weak Duality (Theorem \ref{WeakDuality}).
	In particular, Strong Duality holds for each (Primal Solution, Geometric Multiplier) pair
	\,$(x^{*},\mu^{*})$,\, and
	\begin{equation*}
	x^{*}
	\;\; \in \;\;
		\underset{x\,\in\,\Omega(X,g)}{\argmin}\left\{\,\overset{{\color{white}.}}{f}(x)\,\right\}
		\;\, \bigcap \;\;
		\underset{x\,\in\,X}{\argmin}\left\{\,\overset{{\color{white}.}}{L}(x,\mu^{*})\,\right\}
	\end{equation*}
\end{remark}

\vskip 0.3cm
\proofof Theorem \ref{TheoremSaddlePoint} (Saddle Point Theorem)
\mbox{}
\vskip 0.1cm
\noindent
\underline{($\Longrightarrow$)}
...
\,$(x^{*}\!,\mu^{*})
	...
	\mathcal{G}(f,g)
$,\,
which, by Theorem \ref{PrimalSolutionGeometricMultiplierPairs}, is equivalent to
\begin{equation}
\label{PSGMPCharacterization}
	\left\{\;\;\begin{array}{cl}
		x^{*} \,\in\,\Omega(X,g)\;\bigcap\;\underset{x\,\in X}{\argmin}\;L(x,\mu^{*})\,,
		& \;\textnormal{and}
		\\
		\overset{{\color{white}\textnormal{\large1}}}{\mu_{j}^{*}} \cdot g(x_{j}^{*}) \,=\, 0\,,
		& \;\textnormal{for each \,$j = 1,2,\ldots,r$}
		\end{array}\right.
\end{equation}
Hence, we immediately have
\begin{equation*}
L(x^{*}\!,\mu^{*}) \;\; = \;\; \underset{x\,\in\,X}{\inf}\left\{\,\overset{{\color{white}.}}{L(x,\mu^{*})}\,\right\}
\end{equation*}
Secondly, \,\eqref{PSGMPCharacterization}\, implies
\begin{equation*}
L(x^{*}\!,\mu)
\;\; := \;\;
	f(x^{*}) \, + \,  \mu^{T} \cdot g(x^{*})
...
L(x^{*}\!,\mu^{*})\,,
\quad
\textnormal{for each \,$\mu \in \Re^{r}_{\geq 0}$}\,,
\end{equation*}
...
\;\; =: \;\;
	f^{*}\,,
\end{equation*}
which is equivalent to \,$\mu^{*} \,\in\, \mathcal{G}(f,g)$,\, i.e. \,$\mu^{*}$\, is a geometric multiplier.
It now remains only to show that
\begin{equation*}
x^{*} \;\;\in\;\; \underset{x\,\in\,\Omega(X,g)}{\argmin}\left\{\,\overset{{\color{white}.}}{f}(x)\,\right\}
\end{equation*}
We shall establish this via the following two Claims.

\vskip 0.5cm
\noindent
\textbf{Claim 1:}\quad$x^{*} \,\in\, \underset{x\,\in X}{\argmin}\;L(x,\mu^{*})$.
\vskip 0.0cm
\noindent
Proof of Claim 1:\quad
Immediately by the hypothesis
\,$L(x^{*}\!,\mu^{*}) \, = \, \underset{x\,\in\,X}{\inf}\left\{\,\overset{{\color{white}.}}{L(x,\mu^{*})}\,\right\}$.
This proves Claim 1.
...
\textbf{Claim 2:}\quad
$x^{*} \,\in\, \Omega(X,g)$,\, and
\;$\mu_{j}^{*} \cdot g_{j}(x^{*}) \, = \, 0$,\, for each \,$j = 1,2,\ldots,r$.
\vskip 0.2cm
\noindent
Proof of Claim 2:\quad
Note that, for each \,$x \,\in\, X$,\, we have
\begin{equation*}
\underset{\mu\,\in\Re_{\geq 0}^{r}}{\sup}\left\{\,\overset{{\color{white}.}}{L(x,\mu)}\,\right\}
\;\; = \;\;
	\underset{\mu\,\in\Re_{\geq 0}^{r}}{\sup}\left\{\
...
\;\; = \;\;
	\left\{\begin{array}{cc}
		f(x)\,, & \textnormal{if \,$g(x) \,\leq\, 0$}
		\\
		\overset{{\color{white}1}}{+\infty}\,, & \textnormal{otherwise}
		\end{array}\right.
\end{equation*}
Hence, the hypothesis \eqref{SaddleCondition} implies
\begin{equation*}
\left.\begin{array}{ccccc}
	L(x^{*}\!,\mu^{*})
	& = &
	{\color{white}.}\underset{x\,\in\,X}{\inf}\;\left\{\,\overset{{\color{white}.}}{L(x,\mu^{*})}\,\right\}
	& < &
	+\infty
\\
	L(x^{*}\!,\mu^{*})
	& = &
	\overset{{\color{white}1}}{\underset{\mu\,\in\Re_{\geq 0}^{r}}{\sup}\left\{\,\overset{{\color{white}.}}{L(x^{*}\!,\mu)}\,\right\}}
\end{array}
...
L(x^{*}\!,\mu^{*}) \,=\, f(x^{*})
\end{equation*}
In particular, we see that \,$x^{*} \,\in\, \Omega(X,g)$.
Furthermore, recall that \,$L(x^{*}\!,\mu^{*}) \, := \, f(x^{*}) \,+\, (\mu^{*})^{T} \cdot g(x^{*})$,\,
which, together with \,$L(x^{*}\!,\mu^{*}) \,=\, f(x^{*})$,\,
implies that \,$(\mu^{*})^{T} \cdot g(x^{*})$.\,
Next, observe that
\begin{equation*}
\left.\begin{array}{ccl}
	L(x^{*}\!,\mu^{*}) &:=& f(x^{*}) \,+\, (\mu^{*})^{T} \cdot g(x^{*})
	\\
	\overset{{\color{white}1}}{L(x^{*}\!,\mu^{*})} &=& f(x^{*})
\end{array}
\quad\right\}\!\!\Longrightarrow\quad
0
\;\;=\;\;
	(\mu^{*})^{T} \cdot g(x^{*})
\;\;=\;\;
	\overset{r}{\underset{j=1}{\sum}}\;\, \mu_{j}^{*} \cdot g_{j}(x^{*})
\end{equation*}
Recalling that \,$\mu^{*} \,\geq\, 0$\, and \,$g(^{*}) \,\leq\, 0$,\, we see that
\begin{equation*}
\mu_{j}^{*} \cdot g_{j}(x^{*}) \;\; = \;\; 0\,,
\quad
\textnormal{for each \,$j = 1,2,\ldots,r$}
...
\vskip 0.5cm
\noindent
By Theorem \ref{PrimalSolutionGeometricMultiplierPairs}, Claim 1 and Claim 2 together imply
\begin{equation*}
x^{*} \;\;\in\;\; \underset{x\,\in\,\Omega(X,g)}{\argmin}\left\{\,\overset{{\color{white}.}}{f}(x)\,\right\}
\end{equation*}
This completes the proof of the reverse implication, as well as that of the Theorem.
\qed
...
\vskip 1.0cm
\begin{theorem}[The Dual Problem is convex]
\mbox{}
\vskip 0.1cm
\noindent
