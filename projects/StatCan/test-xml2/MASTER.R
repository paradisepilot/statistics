...
dir.MASTER <- normalizePath(getSrcDirectory(function(x) {x}));
...
source(file.path(dir.code,"xml2csv.R"))

### ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ ###
for (tempXML in list.files(path=dir.data,pattern="query-arXiv")) {
    tempCSV <- gsub(x=tempXML,pattern="\\.txt",replacement='.csv');
    print( paste0( tempXML , "; ", tempCSV ) );
    xml2csv(
         inputXML = file.path(dir.data,tempXML),
        outputCSV = tempCSV
        );
    }
