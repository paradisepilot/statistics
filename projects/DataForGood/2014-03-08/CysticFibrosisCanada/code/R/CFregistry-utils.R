...
load.data <- function() {
	patients         <- read.custom.csv(filename = paste0("tblPatients_", access.date, ".csv"));
	annual.data      <- read.custom.csv(filename = paste0("tblAnnualData_", access.date, ".csv"));
	annual.clinical  <- read.custom.csv(filename = paste0("tblAnnualClinicals_", access.date, ".csv"));
	centres          <- read.custom.csv(file = paste0("tblCentres_", access.date, ".csv"));
	transplants      <- read.custom.csv(filename = paste0("tblTransplants_", access.date, ".csv"));
	disposition      <- read.custom.csv(filename = paste0("tblDisposition_", access.date, ".csv"));
	# provinces      <- read.custom.csv(filename = paste0("tblProvStates_", access.date, ".csv"));
	mutations        <- read.custom.csv(filename = paste0("tblMutations_", access.date, ".csv"));
	cultures         <- read.custom.csv(filename = paste0("tblCultures_", access.date, ".csv"));
	annual.cultures  <- read.custom.csv(filename = paste0("tlinkAnnualCultures_", access.date, ".csv"));
	BCepacia         <- read.custom.csv(filename = paste0("tblBCepacia_", access.date, ".csv"));
	annualBCepacia   <- read.custom.csv(filename = paste0("tlinkAnnualBCepacia_", access.date, ".csv"));
	races            <- read.custom.csv(filename = paste0("tblRaces_", access.date, ".csv"));
	physio           <- read.custom.csv(filename = paste0("tblPhysiotherapy_", access.date, ".csv"));
	annual.physio    <- read.custom.csv(filename = paste0("tlinkAnnualPhysio_", access.date, ".csv"));
	hospitalizations <- read.custom.csv(filename = paste0("tblHospReasons_", access.date, ".csv"));
	annual.hospitalizations <- read.custom.csv(filename = paste0("tlinkAnnualHospitalizations_", access.date, ".csv"));
...
get.all.patients.from.clinic.no.deaths <- function(clinic.id, year) {
	...
	patients.transplant <- unique(
		data$annual.data$PatientKey[data$annual.data$PatientKey %in% data$transplants$PatientKey[format(data$transplants$TransplantDt, "%Y") %in% year] &
		data$annual.data$ReportYr == year & data$annual.data$RptCentreKey %in% clinic.id]
		);
